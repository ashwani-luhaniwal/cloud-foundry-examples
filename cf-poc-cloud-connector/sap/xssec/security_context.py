...
class SecurityContext(object):
    ...
    def _set_subdomain(self, jwt_payload):
        jwt_issuer = jwt_payload.get('iss')
        self._logger.debug(
            'Application received a token with iss: %s.', jwt_issuer)
        if jwt_issuer:
            hostname = urllib.parse.urlparse(jwt_issuer).hostname
            if '.' in hostname:
                self._properties['subdomain'] = hostname[:hostname.index('.')]
    ...
    def _offline_validation(self):
        ...
        self._set_subdomain(jwt_payload)
